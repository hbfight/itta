...
$(function(){
      ...
      var numid = searchURL.replace('?', '').split('&');
      numids=storage.getItem("username_shop");
      if(numids){
        numid=numids;
      }
      ...
      /* 限制要展示的字体 */
       ...
       orderlist_id(num,numid);
       /* 根据id来显示报单信息 */
       function orderlist_id(num,numid){
         $.ajax({
            url:"/api/v1/shopinfo/robdetails/"+numid+"/"+num+"/"+pagesize,
            dataType:"json",
            type:"get",
            success:function(res){
                if(res.msg=="没有数据"){
                   $("#order_page").hide();
                   $("#order_tbody").html('<tr><td colspan="12" style="text-align: center;">暂无你要的数据</td></tr>');
                }else{
                  window.localStorage.setItem("username_shop",numid);
      ...
      function opensettquxiao(){
          layer.open({
            ...
            title:['取消结算'],
            area:['520px','480px'],
            content: '<div id="opensett_div"><p class="opensett_p" id="goodid" style="display:none;"></p><p class="opensett_p"><span>订单号:</span><span class="opensett_span" id="order_number"></span></p><p class="opensett_p"><span>会员账号:</span><span class="opensett_span" id="username"></span></p><p class="opensett_p" id=""><span>支付宝姓名:</span><span class="opensett_span" id="payment_name"></span></p><p class="opensett_p"><span>支付宝账号:</span><span class="opensett_span" id="payment_ac"></span></p><p class="opensett_p"><span>货品名称:</span><span class="opensett_span" id="good_name"></span></p><p class="opensett_p"><span>货品数量:</span><span class="opensett_span" id="quantity"></span></p><p class="opensett_p"><span>货品交易方式:</span><span class="opensett_span" id="mode"></span></p><p class="opensett_p"><span>订单预估结算金额:</span><span class="opensett_span" id="estimate_price"></span></p><form class="layui-form" id="quxiao"><p class="layui-form-item opensett_ps" ><span>商家备注:</span><input type="text" name="business_rm" class="layui-input input-business_rm"></p><p class="layui-form-item opensett_ps"><p class="layui-form-item" table_jiesua_btn><button   class="layui-btn table_jiesua_btnok" lay-filter="quxiao" lay-submit="">确定取消</button></p></form></div>' 
          });
      }
          ...
          form.on('submit(quxiao)', function(data){
             ...
             $.ajax({
                ...
                success:function(res){
                   if(res.code=="201"){
                       layer.msg('取消成功',{icon:1},function(){
                          window.location.href="shoplist_order.html？"+numid;
                       })
                    }else{
                       layer.msg('取消失败',{icon:5},function(){
                          window.location.href="shoplist_order.html?"+numid;
                       })
                    }
                },
                error:function(error){
                   ...
                   layer.msg('取消出错了',{icon:5},function(){
                       window.location.href="shoplist_order.html?"+numid;    
                   })
                }
             });
      ...
      /* 条件选择接口 */
      /*/api/v1/order/ordersortquery*/
      function searchorder_ajax(date,num){
          $.ajax({
            url:"/api/v1/shopinfo/robsortquery/"+date+"/"+num+"/"+pagesize+"/"+numid+"/1",
            dataType:"json",
            type:"get",
            success:function(res){
              console.log(res);	
