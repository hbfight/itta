...
$(function(){
      ...
      var userphone = searchURL.replace('?', '').split('&');

      userphonea=storage.getItem("username_member");
      if(userphonea){
        userphone=userphonea;
      }
      ...
      /* 限制要展示的字体 */
       ...
       $(".rushbuy_address").each(function(){
        ...
        orderlist_id(num,userphone);
       
       /*orderlist_id(num,userphone);*/
       /* 根据id来显示报单信息 */
       function orderlist_id(num,numphone){
         $.ajax({
            url:"/api/v1/uservip/user_order_info/"+numphone+"/"+num+"/"+pagesize,
            dataType:"json",
            type:"get",
            success:function(res){
                ...
                if(res.msg=="没有数据"){
                  ...
                  window.localStorage.setItem("username_member",userphone);
       ...
       function order_ajax_id(num,numphone){
          $.ajax({
            url:"/api/v1/uservip/user_order_info/"+numphone+"/"+num+"/"+pagesize,
            dataType:"json",
            type:"get",
      ...
      layui.use(['form','layer'], function(){
          ...
          form.on('submit(quxiao)', function(data){
            ...
            var quxiaoid=$("#goodid").text();
            console.log(quxiaoid);
            console.log(userphone)
            //发异步，把数据提交给php
            /*quxiao_ajax(id,shunju); */
            /* 取消提交接口方法 */
            $.ajax({
                     url:"/api/v1/order/cancel/"+quxiaoid+"/quxiao",
                     data:shunju,
                     dataType:"json",
                     type:"post",
                     success:function(res){ 
                         ...
                         if(res.code=="201"){ 
                            layer.msg('取消成功',{icon:1},function(){
                               window.location.href="settlement_order.html?"+userphone;
                            })
                     ...
                     error:function(error){
                        console.log(error);
                        layer.msg('取消报错了',{icon:5},function(){
                          window.location.href="settlement_order.html?"+userphone;
                        })
               ...
               $.ajax({
                     ...
                     success:function(res){
                         ...
                         if(res.code=="201"){
                            ...
                            window.location.href="settlement_order.html?"+numphone;
      ...
      /* 条件选择接口 */
      function searchorder_ajax(date,num,numphone){
          console.log(numphone);
          $.ajax({
            url:"/api/v1/shopinfo/robsortquery/"+date+"/"+num+"/"+pagesize+"/"+numphone+"/2",
            dataType:"json",
            type:"get",
            success:function(res){
              console.log(res); 	
       ...
       function page_orderlist_search(date,currentnum){
           $('#orderlistpage').jqPaginator({
                   ...
                   onPageChange: function (num, type) { 
                       ...
                       searchorder_ajaxbody(date,num,userphone);
                       $(".input-page").val("");
                       $("#address_currentnum").text(currentnum);
                       /* 选择页数来跳转页面 */
       ...
       function searchorder_ajaxbody(date,num,numphone){
          $.ajax({
            url:"/api/v1/shopinfo/robsortquery/"+date+"/"+num+"/"+pagesize+"/"+numphone+"/2",
            dataType:"json",
            type:"get",
       ...
       $("#order_selectstatus").change(function(){
            ...
            num=1;
            searchorder_ajax(selectstatus,num,userphone);
            $(".input-page").val("");
       })
       ...
       $("#order_paixu").change(function(){
            ...
            num=1;
            searchorder_ajax(selectpaixu,num,userphone);
            $(".input-page").val("");
       })
